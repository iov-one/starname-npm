import { Coin, decodeBech32Pubkey } from "@cosmjs/amino";
...
export class StarnameApi {
  ...
  public async initialize(
    ...
    if (broker) {
      try {
        this.broker = await Task.toPromise(getIOVAddressForStarname(broker));
      } catch {
        this.broker = "";
      }
    }
  ...
  public getTransactions = (
    address: string,
    page: Pager,
  ): Task<StdMap<ResponsePage<Transaction>>> => {
    const tasksMap: StdMap<Task<ResponsePage<Transaction>>> = queries.reduce(
      (
        map: StdMap<Task<ResponsePage<Transaction>>>,
        createQuery: (address: string) => StargateTxQuery,
      ): StdMap<Task<ResponsePage<Transaction>>> => {
    ...
    return {
      run: async (): Promise<StdMap<ResponsePage<Transaction>>> => {
  ...
  public getPremiumEditionStarnames(
    ownerPromise: string | Promise<string>,
    pageSize: number,
    pageNumber: number,
  ): Task<NameViewData> {
